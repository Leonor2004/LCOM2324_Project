...
int (timer_get_conf)(uint8_t timer, uint8_t *st) {
  ...
  util_sys_inb(timer + 0x40, st);                                   // Call the function util_sys_inb() to read a byte from the timer's register,
...
int (timer_display_conf)(uint8_t timer, uint8_t st, enum timer_status_field field) {
  ...
  timer_print_config(timer, field, conf);  // Call 'timer_print_config' function to display the configuration
